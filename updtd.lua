...
function Window:GetTabMethods()
    local tabMethods = {
        ...
        CreateDropdown = function(self, config)
            ...
            local function CreateDropdownOption(optionConfig)
                ...
                if optionType == "Button" then
                    ...
                    optionButton.MouseButton1Click:Connect(function()
                        currentOption = optionConfig.Text
                        selectedLabel.Text = optionConfig.Text
                        
                        SafeCall(optionConfig.Callback or function() end, optionConfig.Text)
                        
                        if flag and self.Window.Flags then
                            self.Window.Flags[flag] = {CurrentOption = optionConfig.Text}
                        end
                ...
                elseif optionType == "Toggle" then
                    local toggle = tabMethods.CreateToggle(self, optionConfig)
                    toggle.Parent = dropdownList -- Reparent to dropdownList
                    return toggle
                elseif optionType == "Dropdown" then
                    local nestedDropdown = tabMethods.CreateDropdown(self, optionConfig)
                    nestedDropdown.Parent = dropdownList -- Reparent to dropdownList
            ...
            if config.Options then
                for _, optionConfig in pairs(config.Options) do
                    CreateDropdownOption(optionConfig)
                end
            ...
            dropdownButton.MouseButton1Click:Connect(function()
                ...
                if isOpen then 
                    ...
                    CreateTween(dropdownListContainer, {Size = UDim2.new(1, 0, 0, targetSize)}, AnimationSettings.Duration):Play()
                    CreateTween(dropdownList, {Size = UDim2.new(1, 0, 0, targetSize)}, AnimationSettings.Duration):Play()
                else
                    CreateTween(dropdownArrow, {Rotation = 0}):Play()
                    CreateTween(dropdownListContainer, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Duration):Play()
                    CreateTween(dropdownList, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Duration):Play()
                    wait(AnimationSettings.Duration)
                    dropdownList.Visible = false
                end
            end)
