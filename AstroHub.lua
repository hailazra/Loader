...
-- Function to create the main windowfunction AstroHub:CreateWindow(title, sizeX, sizeY, posX, posY, logoAssetId)
    _logoAssetId = logoAssetId    if MainWindow then
...
function AstroHub:SetTheme(themeName)
    ...
    if theme then
        ...
        -- Apply theme to existing elements if GUI is already created
        if MainWindow then
            local MainFrame = MainWindow:FindFirstChild("MainFrame")
            if MainFrame then
                MainFrame.BackgroundColor3 = currentTheme.MainBackground
                ...
                if TitleBar then
                    TitleBar.BackgroundColor3 = currentTheme.TitleBar
                    TitleBar:FindFirstChild("TitleLabel").TextColor3 = currentTheme.TextColor
                    TitleBar:FindFirstChild("CloseButton").TextColor3 = currentTheme.TextColor
                    TitleBar:FindFirstChild("MinimizeButton").TextColor3 = currentTheme.TextColor
                end
                ...
                -- Apply to sections and elements (this will require iterating through all created elements)
                -- For simplicity, we'll assume elements are created after theme is set, or re-apply on theme change.
                -- A more robust solution would involve storing references to all created elements and updating them here.
                -- For now, new elements will pick up the currentTheme.
            end
        end
    else
...
function AstroHub:GetTheme()
    return currentTheme
end

-- Update existing creation functions to use currentTheme
-- (This will be done by modifying the existing functions, not appending new ones)
        ...
        if MainFrame then
            ...
            if TitleBar then
                ...
                if TitleLabel then
                    ...
                    print("AstroHub: Title updated to " .. newTitle)
                else
                    warn("AstroHub: TitleLabel not found.")
                end
            else
                warn("AstroHub: TitleBar not found.")
            end
        else
            warn("AstroHub: MainFrame not found.")
        end
    else
        warn("AstroHub: No GUI window to set title.")
    end
end
...
-- UI Blur Effect (with BlurEffect)
local blurEffect = nil

function AstroHub:SetBlurEffect(enabled)
    if not MainWindow then
        warn("AstroHub: Cannot set blur effect, MainWindow not created.")
        return
    end
    ...
    if enabled then
        if not blurEffect then
            ...
            blurEffect.Name = "AstroHub_BlurEffect"
            blurEffect.Size = 0 -- Start with no blur
            blurEffect.Parent = game.Lighting
        end
        game:GetService("TweenService"):Create(blurEffect, TweenInfo.new(0.5), {Size = 10}):Play()
    else
        if blurEffect then
            game:GetService("TweenService"):Create(blurEffect, TweenInfo.new(0.5), {Size = 0}):Play()
            -- blurEffect:Destroy() -- Optionally destroy after tween, but keeping it allows faster re-enabling
            -- blurEffect = nil
        end
    end
end
