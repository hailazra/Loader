...
function Window:GetTabMethods()
    local tabMethods = {
        ...
        CreateDropdown = function(self, config)
            ...
            local function CreateOption(optionText, level, parentFrame)
                level = level or 0
                parentFrame = parentFrame or dropdownList
                
                local optionButton = Instance.new("TextButton")
                optionButton.Name = optionText
                optionButton.Size = UDim2.new(1, -(level * 15), 0, 22) -- Smaller option height, adjust width for indent
                optionButton.Position = UDim2.new(0, level * 15, 0, 0) -- Indent based on level
                ...
                optionButton.Parent = parentFrame
            ...
            local function CreateSubDropdown(subConfig, level, parentFrame)
                level = level or 0
                parentFrame = parentFrame or dropdownList
                
                local subDropdown = Instance.new("TextButton")
                subDropdown.Name = subConfig.Name or "SubDropdown"
                subDropdown.Size = UDim2.new(1, -(level * 15), 0, 22) -- Smaller option height
                subDropdown.Position = UDim2.new(0, level * 15, 0, 0) -- Indent based on level
                subDropdown.BackgroundColor3 = Theme.Button
                subDropdown.BackgroundTransparency = 0.5
                subDropdown.BorderSizePixel = 0
                subDropdown.Text = "▶ " .. (subConfig.Name or "SubMenu")
                subDropdown.TextColor3 = Theme.Text
                subDropdown.TextSize = 10 -- Smaller font
                subDropdown.Font = Enum.Font.GothamBold
                subDropdown.TextXAlignment = Enum.TextXAlignment.Left
                subDropdown.ZIndex = 22 -- Higher than list
                subDropdown.Parent = parentFrame
                
                CreateCorner(subDropdown, UDim.new(0, 4))
                CreatePadding(subDropdown, UDim.new(0, 6))
                
                local subList = Instance.new("Frame")
                subList.Name = "SubList"
                subList.Size = UDim2.new(1, 0, 0, 0)
                subList.Position = UDim2.new(0, 0, 1, 0)
                subList.BackgroundTransparency = 1
                subList.Visible = false
                subList.Parent = subDropdown
                
                CreateListLayout(subList, Enum.FillDirection.Vertical, UDim.new(0, 2))
                ...
                subDropdown.MouseButton1Click:Connect(function()
                    ...
                    if subOpen then
                        ...
                        CreateTween(subList, {Size = UDim2.new(1, 0, 0, contentHeight)}):Play()
                    else
                        CreateTween(subList, {Size = UDim2.new(1, 0, 0, 0)}):Play()
                        wait(AnimationSettings.Fast)
                        subList.Visible = false
                        subDropdown.Text = "▶ " .. (subConfig.Name or "SubMenu")
                    end
                ...
                -- Create sub-options
                if subConfig.Options then
                    for _, option in pairs(subConfig.Options) do
                        if type(option) == "table" and option.Options then
                            CreateSubDropdown(option, level + 1, subList)
                        else
                            CreateOption(option, level + 1, subList)
                        end
                    end
                end
            ...
            if config.Options then
                for _, option in pairs(config.Options) do
                    if type(option) == "table" and option.Options then
                        CreateSubDropdown(option, 0, dropdownList)
                    else
                        CreateOption(option, 0, dropdownList)
                    end
                end
