...
function Window:GetTabMethods()
    ...
    -- Define CreateDropdownOption as a method of tabMethods
    function tabMethods.CreateDropdownOption(contextSelf, optionConfig, parentList, isNested)
        optionConfig = optionConfig or {}
        local optionType = optionConfig.Type or "Button"
        parentList = parentList or contextSelf.dropdownList -- Default to main dropdownList of the current dropdown

        if optionType == "Button" then
            local optionButton = Instance.new("TextButton")
            optionButton.Name = optionConfig.Text
            optionButton.Size = UDim2.new(1, 0, 0, 22) -- Smaller option height
            optionButton.BackgroundColor3 = Theme.Button
            optionButton.BackgroundTransparency = 0.5
            optionButton.BorderSizePixel = 0
            optionButton.Text = optionConfig.Text
            optionButton.TextColor3 = Theme.Text
            optionButton.TextSize = 10 -- Smaller font
            optionButton.Font = Enum.Font.GothamBold
            optionButton.TextXAlignment = Enum.TextXAlignment.Left
            optionButton.ZIndex = 22 -- Higher than list
            optionButton.Parent = parentList
            
            CreateCorner(optionButton, UDim.new(0, 4))
            CreatePadding(optionButton, UDim.new(0, 6))
            
            optionButton.MouseButton1Click:Connect(function()
                if not isNested then -- Only update selectedLabel for top-level dropdown
                    contextSelf.currentOption = optionConfig.Text
                    contextSelf.selectedLabel.Text = optionConfig.Text
                end
                
                SafeCall(optionConfig.Callback or function() end, optionConfig.Text)
                
                if contextSelf.flag and contextSelf.Window.Flags then
                    contextSelf.Window.Flags[contextSelf.flag] = {CurrentOption = optionConfig.Text}
                end
                
                -- Close dropdown (only top-level)
                if not isNested then
                    contextSelf.isOpen = false
                    CreateTween(contextSelf.dropdownArrow, {Rotation = 0}):Play()
                    CreateTween(contextSelf.dropdownListContainer, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Fast):Play()
                    CreateTween(contextSelf.dropdownList, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Fast):Play()
                    wait(AnimationSettings.Fast)
                    contextSelf.dropdownList.Visible = false
                end
            end)
            
            optionButton.MouseEnter:Connect(function()
                CreateTween(optionButton, {BackgroundColor3 = Theme.ButtonHover, BackgroundTransparency = 0.3}):Play()
            end)
            
            optionButton.MouseLeave:Connect(function()
                CreateTween(optionButton, {BackgroundColor3 = Theme.Button, BackgroundTransparency = 0.5}):Play()
            end)
            return optionButton
        elseif optionType == "Toggle" then
            -- For toggles within dropdowns, we need to ensure they are parented correctly
            -- and their layout order is managed by the dropdown's list layout.
            local toggleFrame = Instance.new("Frame")
            toggleFrame.Name = (optionConfig.Name or "Toggle") .. "Frame"
            toggleFrame.Size = UDim2.new(1, 0, 0, 30) -- Smaller toggle height
            toggleFrame.BackgroundColor3 = Theme.Surface
            toggleFrame.BackgroundTransparency = 0.5
            toggleFrame.BorderSizePixel = 0
            toggleFrame.Parent = parentList -- Parent to the current list
            ...
            toggleLabel.Text = optionConfig.Name or "Toggle"
            ...
            local currentValue = optionConfig.CurrentValue or false
            local flag = optionConfig.Flag
            ...
            local function UpdateToggle(value, animate)
                ...
                SafeCall(optionConfig.Callback or function() end, value)
                
                if flag and contextSelf.Window.Flags then
                    contextSelf.Window.Flags[flag] = {CurrentValue = value}
                end
            end
            ...
            if flag and contextSelf.Window.Flags then
                contextSelf.Window.Flags[flag] = toggleObj
            end
            
            return toggleObj
        elseif optionType == "Dropdown" then
            -- Create a new dropdown frame for the nested dropdown
            local nestedDropdownFrame = Instance.new("Frame")
            nestedDropdownFrame.Name = (optionConfig.Name or "NestedDropdown") .. "Frame"
            nestedDropdownFrame.Size = UDim2.new(1, 0, 0, 30) -- Same height as other options
            nestedDropdownFrame.BackgroundColor3 = Theme.Surface
            nestedDropdownFrame.BackgroundTransparency = 0.5
            nestedDropdownFrame.BorderSizePixel = 0
            nestedDropdownFrame.ClipsDescendants = false
            nestedDropdownFrame.Parent = parentList

            CreateCorner(nestedDropdownFrame, UDim.new(0, 6))
            CreateStroke(nestedDropdownFrame, Theme.Border, 0.5)
            CreatePadding(nestedDropdownFrame, UDim.new(0, 8))

            local nestedDropdownButton = Instance.new("TextButton")
            nestedDropdownButton.Name = "Button"
            nestedDropdownButton.Size = UDim2.new(1, 0, 1, 0)
            nestedDropdownButton.Position = UDim2.new(0, 0, 0, 0)
            nestedDropdownButton.BackgroundTransparency = 1
            nestedDropdownButton.BorderSizePixel = 0
            nestedDropdownButton.Text = optionConfig.Name or "Nested Dropdown"
            nestedDropdownButton.TextColor3 = Theme.Text
            nestedDropdownButton.TextSize = 11
            nestedDropdownButton.Font = Enum.Font.GothamBold
            nestedDropdownButton.TextXAlignment = Enum.TextXAlignment.Left
            nestedDropdownButton.Parent = nestedDropdownFrame

            local nestedSelectedLabel = Instance.new("TextLabel")
            nestedSelectedLabel.Name = "SelectedLabel"
            nestedSelectedLabel.Size = UDim2.new(1, -20, 1, 0)
            nestedSelectedLabel.Position = UDim2.new(0, 0, 0, 0)
            nestedSelectedLabel.BackgroundTransparency = 1
            nestedSelectedLabel.Text = optionConfig.CurrentOption or "Select Option"
            nestedSelectedLabel.TextColor3 = Theme.TextSecondary
            nestedSelectedLabel.TextSize = 10
            nestedSelectedLabel.Font = Enum.Font.GothamBold
            nestedSelectedLabel.TextXAlignment = Enum.TextXAlignment.Right
            nestedSelectedLabel.Parent = nestedDropdownFrame

            local nestedDropdownArrow = Instance.new("TextLabel")
            nestedDropdownArrow.Name = "Arrow"
            nestedDropdownArrow.Size = UDim2.new(0, 15, 1, 0)
            nestedDropdownArrow.Position = UDim2.new(1, -15, 0, 0)
            nestedDropdownArrow.BackgroundTransparency = 1
            nestedDropdownArrow.Text = "▼"
            nestedDropdownArrow.TextColor3 = Theme.TextSecondary
            nestedDropdownArrow.TextSize = 8
            nestedDropdownArrow.Font = Enum.Font.Gotham
            nestedDropdownArrow.Parent = nestedDropdownFrame

            local nestedDropdownListContainer = Instance.new("Frame")
            nestedDropdownListContainer.Name = "NestedListContainer"
            nestedDropdownListContainer.Size = UDim2.new(1, 0, 0, 0)
            nestedDropdownListContainer.BackgroundTransparency = 1
            nestedDropdownListContainer.ClipsDescendants = true
            nestedDropdownListContainer.LayoutOrder = nestedDropdownFrame.LayoutOrder + 1
            nestedDropdownListContainer.Parent = parentList -- Parent to the same list as the nested dropdown frame

            CreatePadding(nestedDropdownListContainer, UDim.new(0, 15, 0, 4)) -- Indent nested options

            local nestedDropdownList = Instance.new("ScrollingFrame")
            nestedDropdownList.Name = "NestedList"
            nestedDropdownList.Size = UDim2.new(1, 0, 1, 0)
            nestedDropdownList.Position = UDim2.new(0, 0, 0, 0)
            nestedDropdownList.BackgroundColor3 = Theme.Surface
            nestedDropdownList.BackgroundTransparency = 0.5
            nestedDropdownList.BorderSizePixel = 0
            nestedDropdownList.Visible = false
            nestedDropdownList.ZIndex = 102 -- Higher than parent dropdown
            nestedDropdownList.ScrollBarThickness = 3
            nestedDropdownList.ScrollBarImageColor3 = Theme.Accent
            nestedDropdownList.CanvasSize = UDim2.new(0, 0, 0, 0)
            nestedDropdownList.Parent = nestedDropdownListContainer

            CreateCorner(nestedDropdownList, UDim.new(0, 6))
            CreateStroke(nestedDropdownList, Theme.BorderAccent, 0.5)
            CreateDropShadow(nestedDropdownList, Vector2.new(0, 2), 8, 0.3)

            local nestedListLayout = CreateListLayout(nestedDropdownList, Enum.FillDirection.Vertical, UDim.new(0, 2))
            CreatePadding(nestedDropdownList, UDim.new(0, 4))

            local nestedIsOpen = false

            local function PopulateNestedDropdown(optionsTable)
                for _, nestedOptionConfig in pairs(optionsTable) do
                    -- PENTING: Teruskan 'contextSelf' dari dropdown induk ke opsi nested
                    tabMethods.CreateDropdownOption(contextSelf, nestedOptionConfig, nestedDropdownList, true)
                end
                nestedListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                    nestedDropdownList.CanvasSize = UDim2.new(0, 0, 0, nestedListLayout.AbsoluteContentSize.Y + 8)
                end)
            end

            if optionConfig.Options then
                PopulateNestedDropdown(optionConfig.Options)
            end

            nestedDropdownButton.MouseButton1Click:Connect(function()
                nestedIsOpen = not nestedIsOpen
                if nestedIsOpen then
                    nestedDropdownList.Visible = true
                    local targetSize = math.min(nestedListLayout.AbsoluteContentSize.Y + 8, 150)
                    CreateTween(nestedDropdownArrow, {Rotation = 180}):Play()
                    CreateTween(nestedDropdownListContainer, {Size = UDim2.new(1, 0, 0, targetSize)}, AnimationSettings.Duration):Play()
                    CreateTween(nestedDropdownList, {Size = UDim2.new(1, 0, 0, targetSize)}, AnimationSettings.Duration):Play()
                else
                    CreateTween(nestedDropdownArrow, {Rotation = 0}):Play()
                    CreateTween(nestedDropdownListContainer, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Duration):Play()
                    CreateTween(nestedDropdownList, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Duration):Play()
                    wait(AnimationSettings.Duration)
                    nestedDropdownList.Visible = false
                end
            end)

            nestedDropdownButton.MouseEnter:Connect(function()
                CreateTween(nestedDropdownButton, {BackgroundColor3 = Theme.ButtonHover, BackgroundTransparency = 0.3}):Play()
            end)

            nestedDropdownButton.MouseLeave:Connect(function()
                CreateTween(nestedDropdownButton, {BackgroundColor3 = Theme.Button, BackgroundTransparency = 0.5}):Play()
            end)

            local nestedDropdownObj = {
                CurrentOption = optionConfig.CurrentOption or "",
                Set = function(option)
                    optionConfig.CurrentOption = option
                    nestedSelectedLabel.Text = option
                end,
                AddOption = function(newOptionConfig)
                    tabMethods.CreateDropdownOption(contextSelf, newOptionConfig, nestedDropdownList, true) -- Call via tabMethods
                end
            }
    ...
    tabMethods.CreateSection = function(self, name)
    ...
    tabMethods.CreateButton = function(self, config)
        config = config or {}
    ...
    tabMethods.CreateToggle = function(self, config)
        config = config or {}
    ...
    tabMethods.CreateSlider = function(self, config)
        config = config or {}
    ...
    tabMethods.CreateDropdown = function(self, config)
        config = config or {}
        ...
        -- Container for dropdown list (will be inserted into the layout)
        local dropdownListContainer = Instance.new("Frame")
        dropdownListContainer.Name = "ListContainer"
        dropdownListContainer.Size = UDim2.new(1, 0, 0, 0) -- Starts collapsed
        dropdownListContainer.BackgroundTransparency = 1
        dropdownListContainer.ClipsDescendants = true -- Clip contents when collapsed
        dropdownListContainer.LayoutOrder = dropdownFrame.LayoutOrder + 1 -- Place right after dropdownFrame
        dropdownListContainer.Parent = self.Frame -- Parent to the same frame as dropdownFrame
        
        local dropdownListLayout = CreateListLayout(dropdownListContainer, Enum.FillDirection.Vertical, UDim.new(0, 2)) -- Layout for options
        dropdownListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Left -- Align options to left
        CreatePadding(dropdownListContainer, UDim.new(0, 15, 0, 4)) -- Indent options

        local dropdownList = Instance.new("ScrollingFrame")
        dropdownList.Name = "List"
        dropdownList.Size = UDim2.new(1, 0, 1, 0) -- Fill container
        ...
        -- Store references to be accessible by CreateDropdownOption
        -- These are now local to this function and passed explicitly where needed
        local dropdownState = {
            ...
            Window = self.Window, -- Pass the main window reference
            tabObject = self -- Pass the tab object itself for nested dropdowns to access tab methods
        }
        
        -- Create options
        if config.Options then
            for _, optionConfig in pairs(config.Options) do
                -- Pass dropdownState as the first argument (contextSelf) to CreateDropdownOption
                tabMethods.CreateDropdownOption(dropdownState, optionConfig, dropdownList) 
            end
        ...
        dropdownButton.MouseButton1Click:Connect(function()
            ...
            if dropdownState.isOpen then 
                dropdownState.dropdownList.Visible = true
                local targetSize = math.min(dropdownState.listLayout.AbsoluteContentSize.Y + 8, 150) -- Max 150px height
                CreateTween(dropdownState.dropdownArrow, {Rotation = 180}):Play()
                CreateTween(dropdownState.dropdownListContainer, {Size = UDim2.new(1, 0, 0, targetSize)}, AnimationSettings.Duration):Play()
                CreateTween(dropdownState.dropdownList, {Size = UDim2.new(1, 0, 0, targetSize)}, AnimationSettings.Duration):Play()
            else
                CreateTween(dropdownState.dropdownArrow, {Rotation = 0}):Play()
                CreateTween(dropdownState.dropdownListContainer, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Duration):Play()
                CreateTween(dropdownState.dropdownList, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Duration):Play()
                wait(AnimationSettings.Duration)
                dropdownState.dropdownList.Visible = false
            end
        end)
        ...
        table.insert(self.Elements, dropdownListContainer) -- Add container to elements to be managed by layout
        return dropdownObj
    end,
    
    tabMethods.CreateInput = function(self, config)
        config = config or {}
