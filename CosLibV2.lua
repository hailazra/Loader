...
function Window:MakeDraggable()
    ...
    -- Draggable for minimized icon
    if self.MinimizeIcon then
        self.MinimizeIcon.InputBegan:Connect(onInputBegan)
    end
end
...
function Window:ToggleMinimize()
    if self.IsMinimized then
        ...
        self:MakeDraggable() -- Call MakeDraggable again to include the MinimizeIcon
    end
end
...
function Window:CreateTab(name, icon)
    ...
    tabButton.Size = UDim2.new(1, -16, 0, 40) -- Adjusted width for better appearance, considering padding
    ...
    CreatePadding(tabFrame, UDim.new(0, 8)) -- Reduced padding
    local layout = CreateListLayout(tabFrame, Enum.FillDirection.Vertical, UDim.new(0, 6)) -- Reduced spacing
...
function Window:SelectTab(targetTab)
    ...
    -- Close all dropdowns when a new tab is selected
    for _, element in pairs(self.CurrentTab.Elements) do
        if element.Name:find("Dropdown") and element.ListContainer.List.Visible then
            local dropdownList = element.ListContainer.List
            local dropdownArrow = element.DropdownArrow
            CreateTween(dropdownArrow, {Rotation = 0}):Play()
            CreateTween(dropdownList.Parent, {Size = UDim2.new(1, 0, 0, 0)}, AnimationSettings.Fast):Play()
    ...
    targetTab.Frame.Visib        CreateTween(targetTab.Button, {
            BackgroundColor3 = Theme.Accent,
            BackgroundTransparency = 0.2,
            TextColor3 = Theme.Text
        }):Play()lf.CurrentTab = targetTab
end
