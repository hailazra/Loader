...
-- Internal menyimpan semua submenu yang terbuka
local openSubmenus = {}
...
local function makeButton(text)
    ...
    b.Font = Enum.Font.SourceSans; b.TextSize = 18; b.TextXAlignment = Enum.TextXAlignment.Left
    b.BorderSizePixel = 0
    return b
end
...
local function makeTextBox(placeholder)
    ...
    tb.BackgroundColor3 = Color3.fromRGB(220,220,255)
    tb.Font = Enum.Font.SourceSans; tb.TextSize = 18
    tb.Text = ""
    return tb
end

-- Nested dropdown (context menu style)
local function makeNested(parentBtn, items)
    local menu = Instance.new("Frame")
    menu.Size = UDim2.new(0,160,0,0)
    menu.AutomaticSize = Enum.AutomaticSize.Y
    menu.Position = UDim2.new(0, parentBtn.AbsoluteSize.X + 4, 0, 0)
    menu.BackgroundColor3 = Color3.fromRGB(250,240,220)
    ...
    for _, item in ipairs(items) do
        local btn = makeButton(item)
        btn.BackgroundColor3 = Color3.fromRGB(240,230,210)
        btn.Parent = menu
    ...
    parentBtn.MouseButton1Click:Connect(function()
        ...
        if menu.Visible then table.insert(openSubmenus, menu) end
    end)

    return menu
end

-- Internal dropdown creator
local function InternalCreateDropdown(parent, title, options)
    ...
    local mainBtn = makeButton("> " .. title)
    ...
    for _, opt in ipairs(options) do
        ...
        elseif t == "toggle" then
            ...
            b.MouseButton1Click:Connect(function()
                state = not state
                b.Text = (state and "[✓] " or "[ ] ") .. opt.text
                if opt.callback then opt.callback(state) end
            end)

        elseif t == "textbox" then
            local tb = makeTextBox(opt.placeholder)
            tb.Parent = content
        ...
        elseif t == "slider" then
            ...
            frame.BackgroundColor3 = Color3.fromRGB(180,180,250)
            frame.Parent = content

            local label = Instance.new("TextLabel", frame)
            label.Text = "Value: " .. tostring(min)
                ...
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    moveConn = UserInputService.InputChanged:Connect(function(i)
                        if i.UserInputType == Enum.UserInputType.MouseMovement then
                            ...
                            local v = math.floor(relX * (max-min) + min)
                            label.Text = "Value: " .. tostring(v)
                            if opt.callback then opt.callback(v) end
                        end
                    end)
                    input.Changed:Connect(function()
                        if input.UserInputState == Enum.UserInputState.End then moveConn:Disconnect() end
                    end)
                end
            end)
        end
        ...
        if opt.nested then
            makeNested(container[#container:GetChildren()], opt.nested)
        end
    end

    mainBtn.MouseButton1Click:Connect(function()
        local open = content.Visible
        if open then
            local tween = TweenService:Create(content, TweenInfo.new(0.25), {Size = UDim2.new(1,0,0,0)})
            tween:Play()
...
-- Public wrapper
function CosmLib:CreateDropdown(parent, title, options)
    return InternalCreateDropdown(parent, title, options)
end
