...
function DropdownComponents.CreateNestedDropdown(parentBtn, items, callback, dropdownId, parentMainFrame)
	...
	local screenGui = parentMainFrame or PlayerGui
	...
	nestedContainer.Size = UDim2.new(0, 250, 1, 0)
	...
	nestedContainer.Parent = screenGui
	...
	local function createMultiSelectButton(item, index)
		...
		indicator.BackgroundColor3 = Theme.Selected
		indicator.BorderSizePixel = 0
		indicator.Visible = false
		indicator.ZIndex = 1004
		indicator.Parent = button

		local function updateState()
			local selected = selectedItems[dropdownId] or {}
			local isSelected = table.find(selected, item) ~= nil
			indicator.Visible = isSelected
			button.BackgroundColor3 = isSelected and Theme.Selected or Theme.Button
		end

		button.MouseButton1Click:Connect(function()
			local selected = selectedItems[dropdownId]
			local i = table.find(selected, item)
			if i then
			...
			updateState()
			if callback then callback(selected, item, not i) end
		end)
	...
	for i, item in ipairs(originalItems) do
		createMultiSelectButton(item, i)
	end

	local function filterItems(text)
		for name, btn in pairs(itemButtons) do
			btn.Visible = text == "" or string.find(string.lower(name), string.lower(text), 1, true)
		end
	end
	...
	local function positionDropdown()
		local abs = parentBtn.AbsolutePosition
		local mainAbs = parentMainFrame.AbsolutePosition
		local relX = abs.X - mainAbs.X
		nestedContainer.Position = UDim2.new(0, relX + parentBtn.AbsoluteSize.X + 4, 0, 0)
		nestedContainer.Size = UDim2.new(0, 250, 1, 0)
	end

	parentBtn.MouseButton1Click:Connect(function()
		local section = parentBtn:FindFirstAncestor("Frame")
		local secId = section and section.Name or "default"

		if not openNestedDropdowns[secId] then
			openNestedDropdowns[secId] = {}
		end

		for id, dropdown in pairs(openNestedDropdowns[secId]) do
			if id ~= dropdownId then dropdown.Visible = false end
		end

		local isVisible = not nestedContainer.Visible
		nestedContainer.Visible = isVisible
		blocker.Visible = isVisible

		if isVisible then
			positionDropdown()
			searchBox.Text = ""
			filterItems("")
			openNestedDropdowns[secId][dropdownId] = nestedContainer
			searchBox:CaptureFocus()
		else
			openNestedDropdowns[secId][dropdownId] = nil
		end
	end)
